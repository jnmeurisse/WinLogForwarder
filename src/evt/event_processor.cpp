...
namespace wlf::evt {
	...
	EventProcessor::EventProcessor(utl::UserCache& user_cache)
		: _renderer(user_cache)
	{
	}
	...
	bool EventProcessor::process(const Context& ctx, const EventLogHandle& event_handle, EventMessageBuilder& message) noexcept
	{
		...
		// Extract system data and xml from the event
	...
	static const wchar_t* sid_name_use_to_string(SID_NAME_USE sid_name_use) noexcept {
		switch (sid_name_use) {
	...
	EventProcessor::Renderer::Renderer(utl::UserCache& user_cache)
		: _render_system_context(EventLogHandle::RenderContext::create_system_context())
		, _user_cache(user_cache)
		, _values_buffer(1 * 1024)
		, _xml_buffer(4 * 1024)
	{
	}


	bool EventProcessor::Renderer::render_event(const EventLogHandle& event_handle, EventData& event_data) noexcept
	{
    ...
    bool EventProcessor::Renderer::render_system_data(const EventLogHandle& event_handle, system_data_t& system_data) noexcept
	{
	...
	void EventProcessor::Renderer::render_user_account(const ::SID* user_id, account_data_t& account_data) noexcept
	{
        account_data.clear();
	...
	bool EventProcessor::Renderer::render_event_xml(const EventLogHandle& event_handle, pugi::xml_document& xml_data) noexcept
	{
        _xml_buffer.clear();
		xml_data.reset();
