	...
	*/
	...
	public:
		...
		*/
		 ...
		 */
		mbed_err configure(const mbedtls_ssl_config& config, mbedtls_net_context& netctx) noexcept;
		 ...
		 * 
		 */
		mbed_err set_hostname(const std::string& hostname) noexcept;

		/**
		 * Notifies the peer that the connection is being closed.
		 *
		 */
		tls_close_status close_notify() noexcept;
		 ...
		 *
		 */
		tls_handshake_status handshake() noexcept;
		 ...
		 * receive operation.
		*/
		rcv_status recv_data(unsigned char* buf, size_t len) noexcept;
		 ...
		 * send operation.
		 */
		snd_status send_data(const unsigned char* buf, size_t len) noexcept;
